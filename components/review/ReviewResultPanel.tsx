...
import { AlertTriangle, CheckCircle2, Info, Lightbulb, Code2, ChevronDown } from "lucide-react"
...
function severityColor(severity: string): "destructive" | "default" | "secondary" | "outline" {
    ...
    <div className="relative size-16 shrink-0">
      <svg className="size-16 -rotate-90" viewBox="0 0 36 36">
        ...
        <circle
          cx="18" cy="18" r="15.5"
          fill="none" stroke="currentColor"
          strokeWidth="3" className={ringColor}
          strokeDasharray={`${pct} ${100 - pct}`}
          strokeLinecap="round"
        />
      </svg>
      <span className={cn("absolute inset-0 flex items-center justify-center text-lg font-bold", ringColor)}>
        {score}
      </span>
    </div>
  )
...
export default function ReviewResultPanel({ result }: { result: ReviewResult }) {
  ...
  const visibleIssues = showAllIssues ? result.issues : result.issues.slice(0, 4)
      ...
      {result.issues.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-2">
            {visibleIssues.map((issue, i) => (
              <div key={i} className="rounded-lg border p-3 space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant={severityColor(issue.severity)} className="text-[11px]">
                    {issue.severity}
                  </Badge>
                  {issue.line != null && (
            ...
            {result.issues.length > 4 && !showAllIssues && (
              <button
                type="button"
              ...
              >
                ...
                Xem thêm {result.issues.length - 4} issues
              </button>
            )}
          </CardContent>
        </Card>
